...
export default function RequestDetailsScreen() {
    ...
    try {
      ...
      if (requestData?.requester_id) {
        const { data: requesterProfile } = await supabase
          ...
          .single();
        if (requesterProfile) {
          requestData.requesterProfile = requesterProfile;
        }
      }
  ...
  const updateDonationStatus = async (
    ...
    try {
      const targetResponse = responses.find((r) => r.id === donationId);
  ...
  const handleDeleteRequest = () => {
    Alert.alert(
      "Delete Request",
      "Are you sure you want to delete this request completely? This action cannot be undone.",
      [
        { text: "Cancel", style: "cancel" },
        { text: "Delete", style: "destructive", onPress: confirmDeleteRequest },
      ],
    );
  };
  ...
  const confirmDeleteRequest = async () => {
    setDeleting(true);
    try {
      // Delete any associated donations first to avoid foreign key constraints
      if (responses.length > 0) {
        await supabase.from("donations").delete().eq("request_id", request.id);
      }
      const { error } = await supabase
        .from("requests")
        .delete()
      ...
      router.replace("/(authenticated)/history");
    } catch (err: any) {
      Alert.alert("Error", err.message);
    } finally {
      setDeleting(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
            ...
            {responses.length === 0 ? (
              <View style={styles.emptyState}>
                <Text style={styles.emptyText}>
                  You haven&apos;t received any donors yet.
                </Text>
              </View>
            ) : (
              <View style={styles.responsesList}>
                {responses.map((resp) => {
                  const isConfirmed = resp.timeline_logs?.some(
                    (l: any) => l.status === "CONFIRMED_BY_REQUESTER",
                  );
                  const needsConfirmation =
                    resp.status === "EN_ROUTE" && !isConfirmed;
                  const displayStatus = needsConfirmation
                    ? "PENDING"
                    : resp.status;
                    ...
                    <View key={resp.id} style={styles.responseCard}>
                      <View style={styles.responseHeader}>
                        ...
                        >
                          ...
                          <Text style={styles.responderName}>
                            {resp.profiles?.full_name || "Unknown Helper"}
                          </Text>
                        </View>
                      ...
                      {resp.profiles?.phone && (
                        <TouchableOpacity
                          ...
                          onPress={() => handleCall(resp.profiles.phone)}
                        >
                      ...
                      <View style={styles.responseStatus}>
                        <Text
                          style={[
                            ...
                            {
                              color: getStatusColor(displayStatus || "PENDING"),
                            },
                          ]}
                        >
                      ...
                      {needsConfirmation && (
                        <View style={styles.actionRow}>
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              updateDonationStatus(resp.id, "CANCELLED")
                            }
                          >
            ...
            {request?.status === "PENDING" &&
              responses.filter((r) => r.status !== "CANCELLED").length ===
                ...
                <TouchableOpacity
                  style={[
                    styles.actionButton,
                    { backgroundColor: COLORS.error, marginTop: SPACING.xl },
                  ]}
        ...
        {isActiveDonor &&
          Object.assign({
            isConfirmed: myDonation?.timeline_logs?.some(
              (l: any) => l.status === "CONFIRMED_BY_REQUESTER",
            ),
            needsConfirmation:
              myDonation?.status === "EN_ROUTE" &&
              !myDonation?.timeline_logs?.some(
                (l: any) => l.status === "CONFIRMED_BY_REQUESTER",
              ),
            displayStatus:
              myDonation?.status === "EN_ROUTE" &&
              ...
              {myDonation?.status === "EN_ROUTE" &&
                ...
                request?.requesterProfile && (
                  <View style={styles.requesterInfo}>
                    ...
                    {request.requesterProfile.phone && (
                      <TouchableOpacity
                        style={[styles.phoneRow, { marginTop: 8 }]}
                        onPress={() =>
                          handleCall(request.requesterProfile.phone)
                        }
                      >
                        <Phone size={16} color={COLORS.primary} />
                        <Text style={styles.phoneText}>Call Requester</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                )}

              {myDonation?.status !== "CANCELLED" &&
                myDonation?.status !== "FULFILLED" && (
                  <TouchableOpacity
          ...
          {isActiveDonor ? (
            ...
            request?.status === "FULFILLED" ? (
              <View
                style={[
                  styles.actionButton,
                  { backgroundColor: COLORS.success, opacity: 0.8 },
                ]}
              >
        ...
        >
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Please let us know why you can&apos;t allowhelp anymore.
            </Text>
